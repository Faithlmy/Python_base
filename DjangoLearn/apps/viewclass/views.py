...
import apps.viewclass.models as models
from apps.viewclass.serializers import CpSerializer
...
class CustomerPaper(
    ...
    這裏可以做接口提示
    """
    queryset = models.TbCustomerPaper.objects.using('customer').all()
    ...
    def get(self, request):
        ...
        if id:
            ...
            m = models.TbCustomerPaper.objects.using('customer').filter()
            my_list = []
            for i in m.values():
                i['st_site'] = "faith"  # 賦值
                i['site'] = i.pop('enabled')  # 改值
    ...
    def post(self, request):
        ...
        if current and isinstance(eval(current), int):
            ...
            models.TbCustomerPaper.objects.using('customer').create(st_site=st_site, current=current, ecn_no=ecn_no)
